...
<template>
  <nav class="p-4 flex justify-between items-center bg-enaBlack text-enaWhite font-ena">
    ...
    <div class="flex items-center space-x-6">
      <!-- Oculta todos os links se hideLinks for true -->
      <template v-if="!hideLinks">
        ...
        <NuxtLink to="/cart" class="relative hover:text-enaYellow">
          ...
          <span
            ...
            class="absolute -top-2 -right-3 inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-black bg-enaYellow rounded-full"
          >
            {{ cart.totalItems }}
          </span>
        </NuxtLink>
        ...
        <template v-if="user">
          <button
            @click="onSignOut"
            class="ml-4 px-3 py-1 border rounded hover:bg-enaYellow hover:text-enaBlack"
          >
            Sair
          </button>
        </template>
